...
if($_GET['action'] == "get" ){
	...
	if(isset($_POST['interest']) && !empty($_POST['interest']) && ($_POST['interest'] !== "all" && $_POST['interest'] !== $all && $_POST['interest'] !== $all_two)) {
		...
		if(count($errors) == 0){
			...
			foreach ($interests as $filter) {
				...
				// Add filters to the showing string
				if(!empty($now_showing)){ $now_showing .= ", "; }
				$interest_qry = mysql_query("SELECT `name` FROM ". TBL_INTEREST ." WHERE `id`=".$filter. " LIMIT 1") or die(error_message("Could not access database", mysql_error(),21));
	...
	if(isset($_POST['program']) && !empty($_POST['program']) && ($_POST['program'] !== "all" && $_POST['program'] !== $all && $_POST['program'] !== $all_two)){
		...
		if(count($errors) == 0){
			...
			foreach ($program as $filter) {
				...
				// Add filters to the showing string
				if(!empty($now_showing)){ $now_showing .= ", "; }
				$program_qry = mysql_query("SELECT `name`, `online` FROM ". TBL_PROGRAM ." WHERE id=$filter LIMIT 1") or die(error_message("Could not access database", mysql_error(),21));
	...
	$count_qry = mysql_query("SELECT COUNT(`profile_id`) FROM ". TBL_DETAILS . " $where_string") 
				or die(error_message("Could not access database", mysql_error(),25));
	...
	$profiles_qry = mysql_query("SELECT *, AES_DECRYPT(en_email, '". SALT ."') AS email 
				...
				or die(error_message("Could not access database", mysql_error(),26));
	...
	elseif(mysql_num_rows($profiles_qry) == 0) {
		...
		while($profile = mysql_fetch_assoc($profiles_qry)) {		
			...
			if(!empty($profile['program_id']) && $profile['position'] === "student"){
				$program_qry = mysql_query("SELECT `name`, `role` FROM ". TBL_PROGRAM ." WHERE id=".$profile['program_id']. " LIMIT 1") or die(error_message("Could not access database", mysql_error(),21));
			...
			if($profile['position'] === "alumni" && !empty($profile['company'])){
				...
				if(!empty($profile['city'])){
					...
					if(!empty($profile['state_id'])){
						$state_qry = mysql_query("SELECT `name` FROM ". TBL_US_STATE ." WHERE id=".$profile['state_id']. " LIMIT 1") or die(error_message("Could not access database", mysql_error(),21));
						$state = mysql_fetch_assoc($state_qry);
						
						$return_content .= ', '. ucwords($state['name']);
					} elseif (!empty($profile['country_id'])){
						$country_qry = mysql_query("SELECT `name` FROM ". TBL_COUNTRY ." WHERE id=".$profile['country_id']. " LIMIT 1") or die(error_message("Could not access database", mysql_error(),21));
			...
			if(!empty($profile['program_id']) && $profile['position'] === "alumni"){
				$program_qry = mysql_query("SELECT `abbreviation`, `name`  FROM ". TBL_PROGRAM ." WHERE id=".$profile['program_id']. " LIMIT 1") or die(error_message("Could not access database", mysql_error(),21));
				$program = mysql_fetch_assoc($program_qry);
			...
			if(!empty($profile['department_id'])){
				$department_qry = mysql_query("SELECT `name` FROM ". TBL_DEPARTMENT ." WHERE id=".$profile['department_id']. " LIMIT 1") or die(error_message("Could not access database", mysql_error(),21));
			...
			if(!empty($profile['comajor_department_id']) ){
				$return_content .= '<br/>';
				$department_qry = mysql_query("SELECT `name` FROM ". TBL_DEPARTMENT ." WHERE id=".$profile['comajor_department_id']. " LIMIT 1") or die(error_message("Could not access database", mysql_error(),21));
				$department = mysql_fetch_assoc($department_qry);
				
				$program_qry = mysql_query("SELECT `name`, `abbreviation` FROM ". TBL_PROGRAM ." WHERE id=".$profile['comajor_program_id']. " LIMIT 1") or die(error_message("Could not access database", mysql_error(),21));
				$program = mysql_fetch_assoc($program_qry);
			...
			$interests_qry = mysql_query("SELECT ". TBL_INTEREST .".name, ". TBL_INTEREST .".id 
										   ...
										   WHERE ".TBL_PROFILE_INTEREST_MAP.".profile_id=".$profile['profile_id']. " ORDER BY ". TBL_INTEREST .".name ASC") or die(error_message("Could not access database", mysql_error(),21));
